...
const eventSchema = z.object({
  nama_event: z.string().min(1, 'Nama event wajib diisi'),
...
async function fetchEvents(): Promise<Event[]> {
  ...
  const { data, error } = await supabase
    .from('event')
    .select('*')
    .order('nama_event', { ascending: true })
...
export default function EventPage() {
  ...
  const [sortField, setSortField] = useState('nama_event')
  ...
  const sortedData = useMemo(
    () => sortEventData(allData, sortField, sortDirection),
  ...
  const form = useForm<EventFormValues>({
    ...
    defaultValues: { nama_event: '', penyelenggara: '' },
  })
  ...
  const createMutation = useMutation({
    mutationFn: (values: EventFormValues) =>
      createEvent({
        ...
        penyelenggara: values.penyelenggara?.trim() || null,
      }),
    ...
    onSuccess: async (result, variables) => {
      ...
      if (userId) {
        await logAudit(
          ...
          'event',
          result.id,
          eventToRecord(variables.oldItem),
          eventToRecord(result)
        )
      }
      invalidate()
  ...
  const openAddDialog = () => {
    setEditingItem(null)
    form.reset({ nama_event: '', penyelenggara: '' })
    setIsFormOpen(true)
  }
  ...
  const openDeleteDialog = (item: Event) => {
  ...
  const columns = useMemo<ColumnDef<Event>[]>(
    () => [
      ...
      {
        accessorKey: 'nama_event',
        header: 'Nama Event',
      },
      {
        accessorKey: 'penyelenggara',
        header: 'Penyelenggara',
        cell: ({ row }) => row.original.penyelenggara ?? '-',
      },
      {
        id: 'actions',
        header: 'Aksi',
        ...
        <DialogContent>
          ...
          >
            <div className="space-y-2">
              ...
              <Input id="nama_event" {...form.register('nama_event')} />
              {form.formState.errors.nama_event && (
                <p className="text-xs text-status-red">
                  {form.formState.errors.nama_event.message}
                </p>
              )}
            </div>
            <div className="space-y-2">
