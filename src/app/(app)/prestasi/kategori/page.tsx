...
import {
  createKategoriPrestasi,
  deleteKategoriPrestasi,
  updateKategoriPrestasi,
} from '@/lib/queries/prestasi'
import { createClient } from '@/lib/supabase/client'
import type { KategoriPrestasi } from '@/lib/supabase/types'
...
function sortKategoriData(
  ...
  const sorted = [...data].sort((a, b) => {
    const aVal = a[sortField as keyof KategoriPrestasi] ?? ''
    const bVal = b[sortField as keyof KategoriPrestasi] ?? ''
    return String(aVal).localeCompare(String(bVal), 'id')
  })
  return sortDirection === 'asc' ? sorted : sorted.reverse()
}

export default function KategoriPrestasiPage() {
  ...
  const [editingItem, setEditingItem] = useState<KategoriPrestasi | null>(null)
  const [deletingItem, setDeletingItem] = useState<KategoriPrestasi | null>(
    null
  )
  ...
  const invalidate = useCallback(() => {
    queryClient.invalidateQueries({ queryKey: ['kategori-prestasi'] })
    queryClient.invalidateQueries({ queryKey: ['kategori-prestasi-list'] })
  }, [queryClient])

  const createMutation = useMutation({
    mutationFn: (values: KategoriFormValues) =>
      createKategoriPrestasi(values.nama_kategori),
  ...
  const updateMutation = useMutation({
    mutationFn: ({
      ...
      oldItem: KategoriPrestasi
    }) => updateKategoriPrestasi(id, values.nama_kategori),
  ...
  const deleteMutation = useMutation({
    mutationFn: (id: string) => deleteKategoriPrestasi(id),
  ...
  const openEditDialog = (item: KategoriPrestasi) => {
    setEditingItem(item)
    form.reset({ nama_kategori: item.nama_kategori })
    setIsFormOpen(true)
  }

  const openDeleteDialog = (item: KategoriPrestasi) => {
  ...
  const columns = useMemo<ColumnDef<KategoriPrestasi>[]>(
    () => [
      {
        id: 'no',
        header: 'No',
